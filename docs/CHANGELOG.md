...
### 🐛 Fix: False Offline Saat Check Banyak Domain

- Browser-side monitoring sebelumnya menjalankan banyak `checkDomainStatus()` sekaligus dengan `Promise.all()`. Saat ratusan domain dicek bersamaan, browser/network bisa membatasi request sehingga banyak domain terbaca `Timeout` 6000 ms walaupun website bisa dibuka manual.
- Tambah helper `checkDomainStatuses()` dengan concurrency limit aman untuk jalur bulk check. Dipakai oleh tab Monitoring, refresh pinned, dan auto-check pinned.

### 🐛 Fix: Website Bisa Dibuka Manual Tapi HEAD Timeout

- `checkHTTPAccess()` kini mencoba `HEAD` lebih dulu, lalu fallback ke `GET`. Beberapa server merespons halaman normal (`GET`) tetapi lambat/menolak `HEAD`, sehingga sebelumnya bisa salah diklasifikasikan offline.
- Timeout browser-side disetel lebih realistis untuk server `.go.id`: DNS lookup 5 detik, HEAD 4 detik, GET 10 detik.
- Status downstream, pin, statistik, export CSV, dan incident logic tetap memakai struktur `DomainStatus` yang sama, jadi fungsi lain tidak berubah kontraknya.
...
- **Root cause:** Logika status di `src/lib/monitoring.ts` menetapkan `dns-only` untuk _semua_ kasus DNS resolve + HTTP gagal, termasuk saat server mati total (`ERR_ADDRESS_UNREACHABLE`, `Failed to fetch`, `Connection Refused`).
...
const handleTogglePin = (id: string) => {
  setDomains((current) =>
    (current || []).map((d) => (d.id === id ? { ...d, pinned: !d.pinned } : d)),
  );
