...
function getRequiredEnv(key) {
  ...
  if (!value || String(value).trim() === "") {
    throw new Error(`[Verify] Missing required environment variable: ${key}`);
  }
...
if (!email || !password) {
  console.error("\n❌ Missing credentials");
  console.error(
    "   Provide --email and --password, or set FIREBASE_CRON_EMAIL/FIREBASE_CRON_PASSWORD\n",
  );
  console.error("   Example:");
  console.error(
    '   node scripts/verify-admin-auth.mjs --email admin@kendal.local --password "your-password"\n',
  );
  process.exit(2);
}
...
async function verify() {
  ...
  const safeNotificationSettings =
    ...
    typeof sharedData.notificationSettings === "object" &&
    !Array.isArray(sharedData.notificationSettings)
      ? sharedData.notificationSettings
      : {
          enabled: false,
          webhookUrl: "",
  ...
  if (printSharedDocKeys) {
    console.log(
      `   users/default-user keys: ${Object.keys(sharedData).join(", ") || "(empty)"}`,
    );
  }
  if (hasSafeEchoPayload) {
    ...
    console.log(
      "   not initialized / invalid shape (write-check may alter data)",
    );
  }
  ...
  if (!hasSafeEchoPayload) {
    ...
    console.log(
      "   Run with --skip-write-check, initialize from UI first, or use --allow-init-write-check.",
    );
    console.log(
      "\n✅ Verification finished (auth + role + read checks passed).\n",
    );
  ...
  try {
    await setDoc(
      sharedUserRef,
      {
        ...
        updatedAt:
          typeof sharedData.updatedAt === "number"
            ? sharedData.updatedAt
            : Date.now(),
      },
      { merge: true },
    );
